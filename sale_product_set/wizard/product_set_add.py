...
class ProductSetAdd(models.TransientModel):
    ...
    def add_set(self):
        ...
        if order_lines:
            ...
            for order_line in self.order_id.order_line.filtered(lambda x: x.product_set_id == self.product_set_id):
                order_line.with_context(wizard_set=self.product_set_id).product_uom_change()
        return order_lines
        ...
        self.env["sale.order.line"].create(
            {
                ...
                'name': self.product_set_id.name,
                'product_set_id': self.product_set_id.id,
                'product_set_qty': self.quantity,
            }
        )
        order_lines = []
    ...
    def prepare_sale_order_line_data(self, set_line, max_sequence=0):
        ...
        sale_line.with_context(set=self.product_set_id).product_id_change()
