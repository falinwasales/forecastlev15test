...
class SaleExcelReportFinalShipping(http.Controller):
    @http.route([
        '/sale/final_shipping_instruction/<model("sale.order"):sale_order>',
    ], type='http', auth="user", csrf=False)
    def get_final_shipping_report(self,sale_order=None,**args):
 
        response = request.make_response(
                    None,
                    headers=[
                        ('Content-Type', 'application/vnd.ms-excel'),
                        ('Content-Disposition', content_disposition('Final Shipping Instruction' + '.xlsx'))
                    ]
                )
        ...
        for sale in sale_order:
            ...
            # ***************************** UPPER TITLE *****************************
            sheet.merge_range('G7:I7', 'FINAL SHIPPING INSTRUCTION', upper_font_title)
            # ***************************** UPPER TITLE *****************************
